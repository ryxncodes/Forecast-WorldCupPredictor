...
export function ThirdPlaceView({ standings, forecast }: { standings: Standings; forecast: Forecast }) {
  ...
  for (const rows of Object.values(standings.groups)) {
    rows.forEach((row, index) => positionByTeam.set(row.team_id, index + 1));
  }
  ...
  return <section className="third-page" aria-labelledby="third-page-heading">
    ...
    <div className="third-page-grid">
      <section aria-labelledby="current-third-heading">
        ...
        <div className="third-ranking-table"><table><thead><tr><th>#</th><th>Team</th><th>Group</th><th>Pts</th><th>GD</th><th>GF</th><th>Status</th></tr></thead><tbody>
          {standings.best_third.map((row, index) => <tr className={index < 8 ? "qualifying" : "outside"} key={row.team_id}><td>{index + 1}</td><th scope="row">{row.team}</th><td>{row.group}</td><td>{row.points}</td><td>{signed(row.goal_difference)}</td><td>{row.goals_for}</td><td><span className={index < 8 ? "status-chip in" : "status-chip out"}>{index < 8 ? "In" : "Out"}</span></td></tr>)}
        </tbody></table></div>
      </section>
      ...
      <section aria-labelledby="projected-third-heading">
        ...
        <div className="third-projection-list">
          {projected.map((row) => {
            ...
            return <div className="third-projection-row" key={row.team_id}><div><strong>{row.team}</strong><span>Group {row.group} · currently {positionByTeam.get(row.team_id) ?? "–"}{positionByTeam.has(row.team_id) ? ["st", "nd", "rd", "th"][Math.min(positionByTeam.get(row.team_id)!, 4) - 1] : ""}</span></div><div className="third-projection-value"><strong>{displayPercent}</strong><span className="probability-track"><span style={{ width: `${Math.max(percent, 1)}%` }} /></span></div></div>;
          })}
        </div>
      </section>
    </div>
    <p className="third-page-note">The current table uses the public score feed. Fair-play card totals and official FIFA ranking are not yet in the dataset, so the app uses its pre-tournament rating only as a final deterministic fallback when every available football criterion is still tied.</p>
  </section>;
}
